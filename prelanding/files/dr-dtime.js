...
function dtimes(d) {
	...
	if (g == 1 || g == 4 || g == 6) {
		...
		document.write((now.getDate()) + " " + months_localized["th"][now.getMonth()]);
	} else if (g == 2 || g == 5 || g == 7) {
		var now = new Date();
...
function dtime_nums(d, like_eu) {
	...
	if (now.getMonth() + 1 < 10) {
  	...
  	if (like_eu === true) {
		document.write(dayNum + "." + monthNum + "." + (now.getFullYear()+543));
    } else {
		document.write(monthNum + "." + dayNum + "." + (now.getFullYear()+543));
    }
